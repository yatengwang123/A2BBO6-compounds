...
score_rf, begin_feature_importance_rf = random_choice_train(df, model_rf, 20)
print(np.array(score_rf).mean())
feature_index_any_rf = reduce(np.union1d, begin_feature_importance_rf)
feature_index_any_rf = np.append(feature_index_any_rf, 'formation_energy')
t2_rf = df[feature_index_any_rf]
my_r2_rf = display_correlation3(t2_rf)
corr_null_name_rf = filter_corr_null(my_r2_rf)
t2_rf.drop(corr_null_name_rf, axis=1, inplace=True)
my_r2_rf.drop(corr_null_name_rf, axis=1, inplace=True)
drop_f2_rf = sperman_rank(my_r2_rf)
drop_f22_rf = drop_corr_formation(drop_f2_rf)
drop_f23_rf = np.array(list(drop_f22_rf))
t2_rf.drop(drop_f23_rf, axis=1, inplace=True)
my_r21 = display_correlation3(t2_rf, save_pig=True, name='RF_Spearman Correlation')
feature, value = feature_importance_dict(model=model_rf, df=t2_rf)
...
ax0.barh(feature[::-1], value[::-1], height=0.8)
ax0.tick_params(direction='in', axis='both', left=True, top=True, right=True, bottom=True, labelleft=True,
                labeltop=False, labelright=False, labelbottom=True)
# plt.show()
plt.savefig('RF_feature_importance.pdf', bbox_inches='tight', pad_inches=0.05)
plt.savefig('RF_feature_importance.png', bbox_inches='tight', pad_inches=0.05, dpi=300)
