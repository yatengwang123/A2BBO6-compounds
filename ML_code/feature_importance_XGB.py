...
import seaborn as sns  # 对于配对图和热图
...
norLocator = AutoMinorLocator()
plt.rc('font', serif='Helvetica', size=25)
plt.rcParams['axes.linewidth'] = 1.5
plt.rcParams['xtick.major.size'] = 6
plt.rcParams['xtick.minor.size'] = 3
plt.rcParams['xtick.major.width'] = 1.50
plt.rcParams['xtick.minor.width'] = 1.50

plt.rcParams['ytick.major.size'] = 6
plt.rcParams['ytick.minor.size'] = 3
plt.rcParams['ytick.major.width'] = 1.5
plt.rcParams['ytick.minor.width'] = 1.50
...
def display_correlation3(df, save_pig=False, name='Spearman Correlation'):
    r = df.corr(method="spearman")
    parameters = {'font.size': 40}
    plt.rcParams.update(parameters)
    plt.figure(figsize=(100, 100))
    ax0 = plt.subplot()
    ax = sns.heatmap(df.corr(method='spearman'), vmin=-1,
                     vmax=1, annot=True, ax=ax0, cmap='RdBu')
    label_y = ax.get_yticklabels()
    plt.setp(label_y, fontsize=50)
    label_x = ax.get_xticklabels()
    plt.setp(label_x, fontsize=50)
    plt.title("Spearman Correlation")
    if save_pig:
        ax0.tick_params(direction='in', axis='both', left=True, top=True, right=True, bottom=True, labelleft=True,
                        labeltop=False, labelright=False, labelbottom=True)
        # plt.show()
        plt.savefig(f'{name}.pdf', bbox_inches='tight', pad_inches=0.05)
        plt.savefig(f'{name}.png', bbox_inches='tight', pad_inches=0.05, dpi=300)
    return (r)
...
print(np.array(score_xgb_gain).mean())
...
my_r2_xgb_gain=display_correlation3(t2_xgb_gain)
...
my_r21_xgb_gain=display_correlation3(t2_xgb_gain,save_pig=True,name='XGB_Spearman Correlation')
t2_xgb_gain.to_excel('XGB_gain_sperman_data.xlsx',index=False)
feature_gain,value_gain=feature_importance_dict(model=model_xgb_gain,df=t2_xgb_gain,name='XGB_gain_feature_importance')
plt.figure(figsize=(20,20))
plt.xticks(fontsize=20)  # 设置x轴刻度字体大小
plt.yticks(fontsize=20)  # 设置y轴刻度字体大小
plt.barh(feature_gain[::-1],value_gain[::-1],height=0.8)
plt.savefig('XGBoost_feature_importane_gain.pdf', bbox_inches='tight', pad_inches=0.05)
plt.savefig('XGBoost_feature_importance_gain.png', bbox_inches='tight', pad_inches=0.05, dpi=300)
...
my_r2_xgb_weight=display_correlation3(t2_xgb_weight)
...
my_r21_xgb_weight=display_correlation3(t2_xgb_weight,save_pig=True,name='XGB_weight Spearman Correlation')
feature,value=feature_importance_dict(model=model_xgb_weight,df=t2_xgb_weight,name='XGB_weight_feature_importance')
print(feature,value)
...
my_r2_xgb_cover=display_correlation3(t2_xgb_cover)
...
my_r21_xgb_cover=display_correlation3(t2_xgb_cover,save_pig=True,name='XGB_cover Spearman Correlation')
feature,value=feature_importance_dict(model=model_xgb_cover,df=t2_xgb_cover,name='XGB_cover_feature_importance')
print(feature,value)
